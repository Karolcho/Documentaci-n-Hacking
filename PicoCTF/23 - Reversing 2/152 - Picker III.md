# Descripción
Can you figure out how this program works to get the flag? Connect to the program with netcat: `$ nc saturn.picoctf.net 61521` The program's source code can be downloaded [here](https://artifacts.picoctf.net/c/525/picker-III.py).
# Solución
Atacar la tabla y introducir la función necesaria para obtener la flag

- Primero descargamos el código que se nos da, y accedemos a la página indicada, después de analizar el código podemos observar que agregaron otra barrera que son las tablas ya definidas para las funciones, pero aprovechándonos de esto introducimos una función con las siguientes líneas, primero accedemos a la opción 3, luego "func_table" y por último le metemos "'win' + ' ' * 125", esto sobrescribe la tabla y coloca que en la primer función se ejecute el win al ejecutarla, para obtener el hexadecimal, traducirlo y tener la flag

picoCTF{7h15_15_wh47_w3_g37_w17h_u53r5_1n_ch4rg3_a186f9ac}


# Notas adicionales
- 
# Referencias
- https://gchq.github.io/CyberChef/#recipe=From_Hex('Auto')&input=